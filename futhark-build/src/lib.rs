...
pub struct Bindings {
    ...
    backend: Backend,
    file: PathBuf,
    configure_cc: Box<dyn Fn(&mut cc::Build)>,
    configure_bindgen: Box<dyn Fn(bindgen::Builder) -> bindgen::Builder>,
}
...
impl Bindings {
    pub fn new<P: Into<PathBuf>>(file: P) -> Self {
        Bindings {
            ...
            configure_cc: Box::new(|_| {}),
            configure_bindgen: Box::new(|b| b),
        }
    }
    ...
    where
        F: Fn(bindgen::Builder) -> bindgen::Builder + 'static,
    {
    ...
    fn compile_c(
        ...
        match self.backend {
            Backend::C => {},
            Backend::Cuda => {
                link_to_native_library(&mut build, "nvrtc")?;
                link_to_native_library(&mut build, "cuda")?;
                link_to_native_library(&mut build, "cudart")?;
            },
            Backend::OpenCL => link_to_native_library(&mut build, "OpenCL")?,
        }
    ...
    fn generate_bindings(
        ...
        let builder = (self.configure_bindgen)(builder);
        ...
        let dest = out_dir.join(format!("{}.bindings.rs", name));
        bindings
            .write_to_file(&dest)
...
impl Display for GeneratorError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            GeneratorError::Command(_) => write!(
                f,
                "Unable to start the futhark compiler. Is it installed?"
            ),
            GeneratorError::BadFilename => {
                write!(f, "The futhark file did not have a UTF-8 name")
            },
